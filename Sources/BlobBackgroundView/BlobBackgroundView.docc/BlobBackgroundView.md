...
Numeric configuration values are normalized to safe ranges. SwiftUI's `animatesTransitions` option controls transition animations for configuration updates; it does not disable the ongoing pulse effect. Use Reduce Motion or set `jellyStrength` to `0` when the background should stay still.
